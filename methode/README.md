<!-- Olivier Godio — https://github.com/oliviergodio-art/turnaround-programmes — CC BY-SA 4.0 -->

# Méthode

> Squelette de travail. Chaque fichier décrit la structure d'une partie et l'objectif de chaque section. Le texte est écrit ensuite, par l'auteur. Les marqueurs indiquent l'état de la matière : **[MATIÈRE OK]** extraite et suffisante, **[À EXTRAIRE]** pratique réelle non encore décrite, **[À CONSTRUIRE]** hors de la pratique actuelle, à concevoir.

## Colonne vertébrale (hypothèse de travail)

**Ce que la méthode fait.** Permettre à quelqu'un qui reprend un programme en dérive — directeur, PMO, consultant — de monter en puissance vite : tous les leviers en main en une semaine, pilote légitime en un mois, pleine puissance en trois mois.

**Ce qu'elle n'est pas.** Une posture. Le lecteur doit savoir quoi faire lundi matin, pas comment se tenir.

**Le mécanisme.** On dirige par la question, pas par l'injonction. Les questions servent des objectifs que l'on a fixés soi-même, après analyse du terrain — main de fer sur les objectifs, gant de velours sur le plan.

**Les quatre niveaux, et leur rigidité.**
- *Objectifs* — rigides (selon leur type ; typologie à écrire).
- *Plan* — fluide, construit avec le terrain, il dévie tout le temps. C'est normal.
- *Jalons* — objectifs intermédiaires, mobiles avec le plan.
- *Vision* — la somme du plan et des objectifs. Une sortie, pas une entrée.

L'erreur type que la méthode corrige : être rigide sur le plan et souple sur l'objectif — pousser un plan mort pendant des mois alors qu'un autre plan servait le même objectif.

**Les trois horizons.**
1. [Semaine 1 — Prendre les leviers en main](01-semaine-1.md)
2. Mois 1 — Devenir le pilote légitime *(à extraire)*
3. Mois 3 — Pleine puissance *(à extraire)*

**Les quatre domaines d'analyse** : scope, parties prenantes, objectifs, risques. Leur contenu change selon l'horizon — en semaine 1, la vision n'existe pas encore (ce sont les *premiers objectifs*) et les risques sont des *doutes sous forme de questions*.

**Nom de la méthode** : non tranché. « PMO inversé » désigne le mécanisme (la question comme instrument de pilotage, le sens du flux inversé au moment de la reprise) ; il est peut-être trop étroit pour une méthode de montée en puissance. À décider quand les trois horizons seront écrits.
